...
export function AppShell({ path, dark, onNavigate, onToggleTheme, children }: Props) {
  ...
  return <div className="app-shell">
    <header className="site-header"><Toolbar><a className="brand" href="/" onClick={(event) => go(event, '/')}><Avatar name="Everton Silva" color="dark-blue" /><span>Everton Silva</span></a><span className="header-spacer" /><Button appearance="subtle" icon={dark ? <LightbulbRegular /> : <DarkThemeRegular />} onClick={onToggleTheme} aria-label="Alternar tema" /></Toolbar></header>
    <nav className="site-nav" aria-label="Navegação principal"><TabList selectedValue={path} onTabSelect={(_, data) => onNavigate(String(data.value))}><Tab value="/" icon={<HomeRegular />}>Visão geral</Tab><Tab value="/projects" icon={<GridRegular />}>Projetos</Tab><Tab value="/stack" icon={<StackRegular />}>Stack & princípios</Tab></TabList></nav>
